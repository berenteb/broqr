...
import {BarElement, CategoryScale, Chart, ChartData, LinearScale, Tooltip} from "chart.js";
...
export const LinkPage: React.FC = () => {
    ...
    const convertedStat = useMemo<ChartData<"bar", number[], string>>(() => {
        return {
            ...
            datasets: [{data: linkStat?.map(ls => ls.value) || [], backgroundColor: theme.colors.theme["500"]}]
        }
    }, [linkStat])
    ...
    const getLink = () => {
        if (token && linkId) {
            ...
            axios
                ...
                .catch(() => {
                    displayMessage('Nem sikerült lekérni a linket.', {type: ErrorTypes.ADMIN})
                })
                .finally(() => {
                    setLoading(false)
                })
    ...
    const getStatistics = () => {
        if (token && linkId) {
            axios
                .get<LinkStatistics[]>(`${API_BASE_URL}/link/statistics/${linkId}/10`, GetAuthHeader(token))
                .then((res) => {
                    setLinkStat(res.data)
                })
                .catch(() => {
                    if (link) displayMessage('Nem sikerült lekérni a link statisztikákat.', {
                        toast: true,
    ...
    return (
        ...
        >
            ...
            {linkStat && (
                <>
                    <Box width="100%" mt={10}>
                        <Bar data={convertedStat} options={{responsive: true, scales: {y: {ticks: {stepSize: 1}}}}}/>
                    </Box>
