...
import { Button, Flex, GridItem, Tag } from "@chakra-ui/react";
...
import { getTierColor } from "../../types/tiers";
...
export const ProfilePage: React.FC = () => {
  ...
  const onDelete = () => {
    if (token)
      axios
        .delete(API_BASE_URL + "/apiKey", GetAuthHeader(token))
        .then(updateUser);
  };
  return (
    <AdminPage heading={user?.username || "Ismeretlen"}>
      <Flex>
        <Tag colorScheme={getTierColor(user?.tier)}>
